...
/**
 * Class to store KDTreeNodes in a k-d tree, a special form of a binary tree
 * built for the quick search of points in a k-dimensional space.
...
public class KDTree<T extends KDTreeNode> {
  ...
  private int axis;
  private ArrayList<T> kdTreeNodes;
   ...
   */
  public KDTree(ArrayList<T> kdTreeNodes, int depth) {
    ...
    if (size == 0) {
      ...
      this.left = new KDTree<>(new ArrayList<>(kdTreeNodes.subList(0, medianIndex)),
              depth + 1);
      this.right = new KDTree<>(new ArrayList<>(kdTreeNodes.subList(medianIndex + 1, size)),
              depth + 1);
    }
  }

  /**
   * Performs either a radius search or a k-nearest neighbors search using a specific
   * k-d tree traversal algorithm.
   ...
   * @return PriorityQueue in descending order by distance from targetPoint of
   * nearest neighbors/nodes within radius.
   */
  public PriorityQueue<T> kdTreeSearch(String type, double val, KDTreeNode targetPoint,
                                       Comparator<KDTreeNode> distanceSorter, HashSet<T> ignore) {
  ...
  /**
   * Helper function to recursively to perform either a radius search or
   * a k-nearest neighbors search.
   * @param type The type of search to perform. Can either be "neighbors" or "radius."
   * @param val If type is "neighbors", the number of neighbors to return. If type is "radius", the
   *            radius around targetPoint to search for nodes.
   * @param returnNodes PriorityQueue in descending order by distance from targetPoint of
   *                    nearest neighbors/nodes within radius.
   * @param targetPoint The point to search around for neighbors or points in a given radius.
   * @param ignore HashSet of KDTreeNodes to skip when returning nodes. Used to avoid returning
   *               "name" when command format is 'neighbors k "name"' OR 'radius r "name"'.
   */
  private void kdTreeSearchRecursive(String type, double val, PriorityQueue<T> returnNodes,
                                     KDTreeNode targetPoint, HashSet<T> ignore) {
    if (this.getHead() != null) {
      ...
      if ("neighbors".equals(type)) {
        ...
        if (!ignore.contains(currentNode)
              && Double.compare(currentDistance, val) <= 0) {
  ...
  /**
   * Performs either a radius search or a k-nearest neighbors search using a naive,
   ...
   * nearest neighbors/nodes within radius.
   */
  public PriorityQueue<T> naiveSearch(String type, double val, KDTreeNode targetPoint,
                                      HashSet<T> ignore) {
    ArrayList<T> nodeList = this.kdTreeNodes;
    int k = (int) Math.round(val);
    ...
    if (type.equals("radius")) {
      ...
      if (k == 0 || nodeList.isEmpty()) {
        ...
        for (T node : nodeList) {
          ...
          if (!ignore.contains(node)) {
            double currentDistance = targetPoint.euclideanDistance(node);
            if (returnNodes.size() == k && Double.compare(currentDistance,
                  returnNodes.peek().euclideanDistance(targetPoint)) <= 0) {
              returnNodes.poll();
              returnNodes.add(node);
            } else if (returnNodes.size() < k) {
              returnNodes.add(node);
   ...
   */
  public ArrayList<T> getKdTreeNodes() {
